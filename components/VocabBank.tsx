...
export default function VocabBank({ vocabList, onRefreshVocab }: VocabBankProps) {
  ...
  const [deletingVocabItem, setDeletingVocabItem] = useState<VocabItem | null>(null);
  ...
  const handleDelete = (item: VocabItem) => {
    setDeletingVocabItem(item);
  };

  const handleConfirmDeleteVocab = async () => {
    if (deletingVocabItem) {
      await deleteVocab(deletingVocabItem.id);
      setDeletingVocabItem(null);
      onRefreshVocab();
    }
  };
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  This word will be removed from your vocab bank.
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 p-3.5 rounded-2xl text-xs text-slate-700 dark:text-slate-300">
              Are you sure you want to remove <strong>&ldquo;{deletingVocabItem.word}&rdquo;</strong> from your vocabulary bank?
            </div>

            <div className="flex items-center gap-2 pt-2">
              <button
                type="button"
                onClick={() => setDeletingVocabItem(null)}
              ...
              <button
                type="button"
                onClick={handleConfirmDeleteVocab}
