        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {stage === 'testing' && currentItem && (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between gap-2 text-xs font-semibold text-slate-500 pb-2 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-slate-900 dark:text-white">
                    Question {currentIndex + 1} of {testDeck.length}
                  </span>
                  <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-[11px] text-slate-600 dark:text-slate-300">
                    {currentItem.subject} • {currentItem.subTopic}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  {timerMinutes > 0 && (
                    <span className="font-mono font-bold text-amber-500 flex items-center gap-1">
                      ...
                      {Math.floor(overallTimeLeft / 60)}m {overallTimeLeft % 60}s
                    </span>
                  )}
                  ...
                  >
                    <PenTool className="w-3.5 h-3.5" /> Scratchpad
                  </button>
                </div>
              </div>
              ...
              {currentItem.imageDataUrl && (
                <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold"
                  >
                    <ImageIcon className="w-4 h-4 text-blue-500" />
                    <span>{showScreenshot ? 'Hide Screenshot' : 'View Original Screenshot'}</span>
                  </button>
                  <span className="text-[11px] text-slate-400 italic">Optional reference</span>
                </div>
              )}

              {showScreenshot && currentItem.imageDataUrl && (
                <div className="p-2 rounded-xl bg-slate-950 border border-slate-800 max-h-64 overflow-hidden flex justify-center">
                  <img
              ...
              {currentItem.answerChoices && currentItem.answerChoices.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {currentItem.answerChoices.map((choice) => {
                    ...
                    if (isQuestionAnswered && examStyle === 'instant') {
                      if (isCorrect) {
                        bg =
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          handleAnswerQuestion(choice.label);
                        }}
                        className={`p-3.5 rounded-xl border-2 text-left text-sm transition-all flex items-start gap-3 ${bg}`}
                      >
                        ...
                        {isQuestionAnswered && examStyle === 'instant' && isCorrect && (
                          <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" />
                        )}
                        {isQuestionAnswered && examStyle === 'instant' && isSelected && !isCorrect && (
                ...
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') handleAnswerQuestion(typedResponse);
                      }}
                      placeholder="Type answer here..."
                      className="flex-1 p-3 text-sm rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    />
                    <button
                      type="button"
                      onClick={() => handleAnswerQuestion(typedResponse)}
                      className="px-5 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs"
                    >
                      Check
                    </button>
                  </div>
              ...
              {/* Scratchpad */}
              {showScratchpad && <Scratchpad height={180} />}

              {/* Instant AI Explanation Banner */}
              {isQuestionAnswered && examStyle === 'instant' && (
                <div className="p-4 rounded-2xl bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 text-xs text-amber-950 dark:text-amber-200 space-y-2 animate-in fade-in duration-200">
                  <div className="font-extrabold flex items-center gap-1.5 text-amber-800 dark:text-amber-300">
                    <Sparkles className="w-4 h-4 text-amber-500" />
                    <span>AI Takeaway & Explanation:</span>
                  </div>
                  <div className="font-semibold text-amber-900 dark:text-amber-100">
                    <MathRenderer text={currentItem.aiTakeaway} />
                  </div>
                  <div className="text-slate-700 dark:text-slate-300 leading-relaxed pt-1 border-t border-amber-200/60 dark:border-amber-900/40">
              ...
              <div className="pt-2 flex items-center justify-between">
                <button
                  ...
                  className="px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold text-xs disabled:opacity-40"
                >
                  Previous
                </button>
                ...
                >
                  <span>
                    {currentIndex + 1 < testDeck.length ? 'Next Question' : 'Complete & View Grade'}
                  </span>
